...
const coverGap = (index, total, simplify) => {
  ...
  // Tighter gaps on mobile = less sticky scroll distance / jank
  if (simplify) {
    if (fromEnd === 1) return 4;
    if (fromEnd === 2) return 5;
    return Math.max(6, 10 - index);
  }
...
/** Mobile: sticky deck UI kept, no per-card scroll scrub listeners */
const WorkCardStatic = ({ work, index, total, simplify }) => {
...
/** Desktop: sticky + subtle scale/overlay as next card covers */
const WorkCardMotion = ({ work, index, total }) => {
  const cardRef = useRef(null);
  const isLast = index === total - 1;
  const gap = coverGap(index, total, false);
  const trackScroll = !isLast && index < total - 2;
    ...
    <>
      ...
      >
        <WorkCardShell
          work={work}
          index={index}
          scaleStyle={trackScroll ? { scale } : undefined}
          overlayOpacity={overlayOpacity}
          showOverlay={trackScroll}
        />
      </div>
      {gap > 0 && (
        <div
...
const WorkCard = (props) => {
  ...
  if (simplify || reduceMotion) {
    return <WorkCardStatic {...props} simplify={simplify} />;
  }
  return <WorkCardMotion {...props} />;
};
